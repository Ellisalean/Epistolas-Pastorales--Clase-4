...
import { PASTORAL_LESSON_4 } from './data/lessonContent';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (isPlaying) {
      interval = window.setInterval(() => {
        if (currentSlideIndex < PASTORAL_LESSON_4.totalSlides - 1) {
          nextSlide();
        } else {
          toggleAutoplay();
        }
  ...
  const currentSlide = PASTORAL_LESSON_4.slides[currentSlideIndex];
  const total = PASTORAL_LESSON_4.totalSlides;
      ...
      `}>
        <div className="h-full flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-1 custom-scrollbar">
            {PASTORAL_LESSON_4.slides.map((s, idx) => (
      ...
      <main className="flex-1 flex flex-col h-full relative bg-[#111111]">
        <header className="h-16 px-6 lg:px-10 flex items-center justify-between border-b border-white/5 bg-[#1a1a1a] z-50 shrink-0">
           <div className="flex items-center gap-6">
              ...
              <h1 className="hidden sm:block text-xs font-black uppercase tracking-widest text-white/80">{PASTORAL_LESSON_4.subtitle}</h1>
           </div>
